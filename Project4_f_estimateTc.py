# Read data from all the different txt-files and make a lest square
...
T1, E1, CV1, Chi1, absM1 = read_quantities('Ising2D_f_L20.txt')
T2, E2, CV2, Chi2, absM2 = read_quantities('Ising2D_f_L40.txt')
T3, E3, CV3, Chi3, absM3 = read_quantities('Ising2D_f_L60.txt')
T4, E4, CV4, Chi4, absM4 = read_quantities('Ising2D_f_L80.txt')
...
one_over_L_list = [1.0/20,1.0/40,1.0/60,1.0/80]
...
ax.plot(one_over_L_list, T_list,'go',label='Data points from $C_V$')
ax.plot(reciprocal_refined, y,'b-',label='Least-squares fit, $T_c(\infty) = %.3f$' % TC_b)
ax.legend(loc='best',fancybox='True',shadow='True')
ax.set_ylim([TC_b-0.01,max(T_list)+0.01])
ax.set_xlabel('Reciprocal system size, $1/L$')
ax.set_ylabel('Maxima of heat capacity, $\max{(C_V(L))}$')
ax.set_title('Relation between reciprocal system size and critical temperature') 
ax.grid()
plt.savefig('Project4_f_criticaltermp.eps', format='eps', dpi=1000)
plt.show()